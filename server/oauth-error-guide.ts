...
const OAUTH_ERROR_GUIDES: Record<string, OAuthErrorGuide> = {
  ...
  invalid_client: {
    ...
    description: "Client authentication failed or client credentials are invalid.",
    troubleshooting: [
  ...
  invalid_request: {
    ...
    description: "The request is missing a required parameter, includes invalid values, or is malformed.",
    troubleshooting: [
  ...
  temporarily_unavailable: {
    ...
    description: "The authorization server is currently unable to handle the request due to temporary overloading.",
    troubleshooting: [
...
export function enhanceOAuthError(
  ...
  try {
    ...
    errorData = {
      error: params.get("error") || params.get("error_description") || errorText,
  ...
  const enhancedError: EnhancedOAuthError = {
    error: errorDescription || `OAuth error: ${errorCode}` || "Authentication failed",
  };
