import { enhanceOAuthError, isKnownOAuthError, getAllErrorGuides } from "../oauth-error-guide";

describe("OAuth Error Guide", () => {
  describe("enhanceOAuthError", () => {
    it("should detect invalid_scope error and provide guidance", () => {
      const errorResponse = JSON.stringify({ error: "invalid_scope", error_description: "The requested scope is invalid" });
      ...
      expect(enhanced.guide?.troubleshooting).toContain("Check that all requested scopes are supported by the OAuth provider");
    });

    it("should detect unauthorized_client error and provide guidance", () => {
      const errorResponse = JSON.stringify({ error: "unauthorized_client", error_description: "Client authentication failed" });
      ...
      expect(enhanced.guide?.troubleshooting).toContain("Verify your Client ID and Client Secret are correct");
    });

    it("should detect access_denied error and provide guidance", () => {
      const errorResponse = JSON.stringify({ error: "access_denied", error_description: "User denied the request" });
      ...
      expect(enhanced.guide?.troubleshooting).toContain("User may have clicked 'Cancel' or 'Deny' during authorization");
    });

    it("should handle URL-encoded error responses", () => {
      const errorResponse = "error=invalid_grant&error_description=The%20authorization%20code%20has%20expired";
    ...
    it("should handle unknown error codes gracefully", () => {
      const errorResponse = JSON.stringify({ error: "unknown_error", error_description: "This is an unknown error" });
