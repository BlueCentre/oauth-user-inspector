...
const ApiExplorer: React.FC<ApiExplorerProps> = ({ user }) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-slate-200 mb-2">API Explorer</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Endpoints List */}
        <div>
          <h4 className="text-md font-medium text-slate-300 mb-3">Available Endpoints</h4>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-400 mb-2">{endpoint.description}</p>
                      {endpoint.requiredScopes && (
                        <div className="text-xs">
                          <span className="text-slate-500">Required scopes: </span>
                          <span className={hasScopes ? "text-green-400" : "text-red-400"}>
                            {endpoint.requiredScopes.join(", ")}
                          </span>
                        </div>
                      )}
                    </div>
                    {isLoading && isSelected && <Spinner className="w-4 h-4 text-blue-400" />}
                  </div>
                </button>
              );
            })}
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 border border-slate-600 bg-slate-800/50 rounded-md">
                <div className="flex items-center justify-between mb-2">
                  <h5 className="font-medium text-slate-200">Response Details</h5>
                ...
                {selectedEndpoint && (
                  <div className="text-xs text-slate-400 space-y-1">
                    <div><span className="font-medium">URL:</span> {selectedEndpoint.url}</div>
              ...
              {response.data && (
                <div className="border border-slate-600 bg-slate-800/50 rounded-md">
                  <div className="p-3 border-b border-slate-600">
                    <h5 className="font-medium text-slate-200">Response Data</h5>
                  </div>
                  <div className="p-3">
                    <JsonTree 
                      data={response.data} 
